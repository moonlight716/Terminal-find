...
TFIND_BASH_BLOCK_START = "# >>> tfind >>>"
TFIND_BASH_BLOCK_END = "# <<< tfind <<<"
...
def config_root() -> Path:
    ...
    if override:
        return Path(override).expanduser() / "tfind"
    return Path.home() / ".config" / "tfind"


def bash_config_path() -> Path:
    override = os.environ.get("TFIND_CONFIG_FILE")
    if override:
        return Path(override).expanduser()
    return config_root() / "config.sh"


def bashrc_path() -> Path:
    override = os.environ.get("TFIND_BASHRC_PATH")
    if override:
        return Path(override).expanduser()
...
def build_bootstrap_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--install",
        action="store_true",
        help="Write local shell config and update ~/.bashrc for the selected shell.",
    )
    parser.add_argument(
        "--python",
        dest="python_path",
        help="Absolute Python interpreter path to pin for Bash integration.",
    )
    return parser
...
def run_doctor() -> int:
    ...
    print(f"bash config: {bash_config}")
    print(f"bash config exists: {'yes' if bash_config.exists() else 'no'}")
    print(f"bash rc: {bashrc_path()}")
    print(f"TFIND_PYTHON: {os.environ.get('TFIND_PYTHON', '(not set)')}")
...
def _validate_python_path(python_path: str) -> Path | None:
    candidate = Path(python_path).expanduser()
    if not candidate.is_absolute():
        print(f"--python must be an absolute path: {python_path}", file=sys.stderr)
        return None
    if not candidate.exists():
        print(f"Python interpreter does not exist: {candidate}", file=sys.stderr)
        return None
    if not os.access(candidate, os.X_OK):
        print(f"Python interpreter is not executable: {candidate}", file=sys.stderr)
        return None
    return candidate.resolve()


def _bash_install_block(script_path: Path) -> str:
    return "\n".join(
        [
            TFIND_BASH_BLOCK_START,
            f"source {shlex.quote(str(script_path))}",
            TFIND_BASH_BLOCK_END,
            "",
        ]
    )


def _write_bash_config(python_path: Path, repo: Path) -> Path:
    config_path = bash_config_path()
    config_path.parent.mkdir(parents=True, exist_ok=True)
    content = "\n".join(
        [
            "# Generated by `tfind bootstrap bash --install`.",
            f"export TFIND_PYTHON={shlex.quote(str(python_path))}",
            f"export TFIND_REPO_ROOT={shlex.quote(str(repo))}",
            "",
        ]
    )
...
def _update_bashrc(script_path: Path) -> Path:
    bashrc = bashrc_path()
    existing = bashrc.read_text(encoding="utf-8") if bashrc.exists() else ""
    block_pattern = re.compile(
        rf"(?ms)^{re.escape(TFIND_BASH_BLOCK_START)}\n.*?^{re.escape(TFIND_BASH_BLOCK_END)}\n?"
    )
    cleaned = block_pattern.sub("", existing).rstrip("\n")
    block = _bash_install_block(script_path)
    updated = block if not cleaned else f"{cleaned}\n\n{block}"
    bashrc.parent.mkdir(parents=True, exist_ok=True)
    bashrc.write_text(updated, encoding="utf-8")
    return bashrc


def _install_bash_bootstrap(python_path: str) -> int:
    interpreter = _validate_python_path(python_path)
    if interpreter is None:
        return 2

    repo = repo_root()
    script_path = repo / "integrations" / "bash" / "tfind.bash"
    config_path = _write_bash_config(interpreter, repo)
    bashrc = _update_bashrc(script_path)

    print("Installed Bash integration.")
    print(f"config: {config_path}")
    print(f"python: {interpreter}")
    print(f"repo: {repo}")
    print(f"bash rc: {bashrc}")
    print(f"source script: {script_path}")
    return 0


def run_bootstrap(shell: str, path_only: bool, install: bool, python_path: str | None) -> int:
    if shell == "powershell":
    ...
    if install:
        if path_only:
            print("--install cannot be combined with --path-only.", file=sys.stderr)
            return 2
        if shell != "bash":
            print("`tfind bootstrap --install` is currently only supported for Bash.", file=sys.stderr)
            return 2
        if not python_path:
            print("`tfind bootstrap bash --install` requires `--python /absolute/path/to/python`.", file=sys.stderr)
            return 2
        return _install_bash_bootstrap(python_path)
